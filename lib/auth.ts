...
export async function destroySession() {
  ...
  if (token && process.env.DATABASE_URL) {
    await ensureSchema();
    await requireDb()`DELETE FROM sessions WHERE token_hash=${tokenHash(token)}`;
  }
  jar.delete(COOKIE);
}
